...
def tori_blend_tsdf(Ps, Ns, tori_net, res=48, trunc=0.1, bound=1.1, device="cpu",
    ...
    ``Ps``/``Ns`` are ``(B,N,3)`` (or a single ``(N,3)``).  Runs ``tori_net`` (a
    ``CoeffNet``) on each cloud's kNN neighborhoods → per-point tori → the self-
    normalized blend (Eq. 25) evaluated on the grid, clipped to ``±trunc``.  Returns
    ``(B,1,res,res,res)`` in distance units (``<0`` inside).
    """
    ...
    params = core.coeffs_to_torus(Ps, Ns, coeffs)            # batched per-point tori
    c0 = params["center"].unsqueeze(1); u0 = params["axis"].unsqueeze(1)
    R0 = params["R"].unsqueeze(1); r0 = params["r"].unsqueeze(1); s0 = params["sign"].unsqueeze(1)
    ...
    for a in range(0, G, qchunk):
        ...
        g = s0 * core.torus_sdf(x.unsqueeze(2), c0, u0, R0, r0)      # (B,q,N)
...
class WaveToriReconstruction:
    ...
    def __init__(self, P, N, tori_net, wave_net, *, res=48, trunc=0.1, bound=1.1,
        ...
        with torch.no_grad():
            prior = tori_blend_tsdf(P, N, tori_net, res, trunc, bound, device, k) / trunc
